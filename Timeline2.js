// Set the dimensions and margins of the graph
        var margin = {top: 30, right: 130, bottom: 40, left: 90}, // Increased right margin
...
var colorScale = d3.scaleOrdinal(d3.schemeCategory10)
        ...
        var legend = svg.selectAll(".legend")
            ...
            .enter().append("g")
            .attr("class", "legend")
            .attr("transform", (d, i) => `translate(${width}, ${i * 20})`); // Move to top right
   ...
   // Helper function to parse year ranges and return start years and end years
function parseYearRanges(yearRanges) {
  let years = yearRanges.map(range => {
    let [start, end] = range.split('-').map(Number);
    if (isNaN(end)) { // If end year is not available or valid
      end = start; // Consider range as a single year
    }
    return {start, end};
  });
...
// Extract all year ranges and flatten them into a list of years
let allYears = [];
data.forEach(countryData => {
  let yearRanges = parseYearRanges(Object.keys(countryData.key_historical_events));
  yearRanges.forEach(({start, end}) => {
    allYears.push(start);
    if (end !== start) { // Only add end if it's different from start
      allYears.push(end);
    }
  });
});
...
// Set up the scales with the new domain
var xScale = d3.scaleLinear()
 ...
 var yScale = d3.scaleBand()
    .domain(data.map(d => d.country))
    .range([0, height])
    .paddingInner(0.3) // Adjust padding to increase space between bars
    .paddingOuter(0.2); // You can also adjust outer padding if needed
...
svg.append("text")
    .attr("x", width / 2)
    .attr("y", -10) // Increase the space by setting a larger negative number
