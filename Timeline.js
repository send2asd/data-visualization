// The JSON data
    ...
    },
    {
       ...
       // Parse the years into dates
       var parseYear = d3.timeParse("%Y");

       var margin = {top: 0, right: 20, bottom: 50, left: 50}, // Increase right margin
    width = 1500 - margin.left - margin.right,
    height = 500 - margin.top - margin.bottom; // Adjust height as needed
...
var xScale = d3.scaleTime()
    .range([0, width - 500]); // Provide some padding on the right side
...
// Initialize timeline with the first country data
updateTimeline(data[0].country);

function updateTimeline(country) {

    //updating the title
    d3.select("#title_span").text(country);
    var countryData = data.find(d => d.country === country);
    var events = Object.entries(countryData.key_historical_events).map(([year, event]) => ({
        date: parseFirstYear(year),
        event
    }));
    ...
    events.sort((a, b) => a.date - b.date);
     ...
     svg.selectAll(".event-circle").remove();
    svg.selectAll(".event-text").remove();

    // Define the domain of the scale based on the data
    var yearExtent = d3.extent(events, function(d) { return d.date; });
    xScale.domain(yearExtent);
    xAxis.transition().call(d3.axisBottom(xScale));

    // Calculate spacing based on the number of events and the height of the canvas
    var totalHeightAvailable = height - margin.top - margin.bottom - 35; // 20 for padding above the x-axis
    var spacing = Math.max(totalHeightAvailable / events.length, 35); // At least 20 pixels between events

    // Assign unique y-positions with collision avoidance
    var lastY = margin.top; // Start from the top of the canvas
    events.forEach(function(d, i) {
        // Increase yPos only if the date is different than the previous one to prevent overlap
        if (i > 0 && d.date.getTime() === events[i - 1].date.getTime()) {
            d.y = events[i - 1].y; // Use the same y position for the same year to prevent overlap
        } else {
            lastY += spacing;
            d.y = lastY; // Assign the yPos to the y property of the event
        }
    });

    // Ensure the last event is above the x-axis
    if (events[events.length - 1].y + spacing > height - margin.bottom) {
        let excessHeight = events[events.length - 1].y + spacing - (height - margin.bottom);
        events.forEach(function(d) {
            d.y -= excessHeight; // Shift all y positions up by the excess amount
        });
        // Define a y-scale (this assumes that your events are spaced evenly as per your 'spacing' variable)
    var yScale = d3.scaleLinear()
        .domain([0, d3.max(events, function(d) { return d.y; })])
        .range([height, margin.top]);

    // Create a y-axis with the y-scale
    var yAxis = d3.axisLeft(yScale).ticks(5).tickSize(0).tickFormat("");

    // Update the y-axis
    var yAxisGroup = svg.selectAll(".y-axis").data([0]); // Bind a dummy data array for enter/update pattern

    yAxisGroup.enter()
        .append("g")
        .attr("class", "y-axis")
        .merge(yAxisGroup) // Combine enter and update selections
    ...
    // Data-join (enter, update, exit) for the event circles
    var eventCircles = svg.selectAll(".event-circle")
        .data(events, function(d) { return d.date; }); // Use date as key for binding data

    // Create new event circles (assuming no circles exist now since we've removed them)
    ...
    // Update existing event circles
    eventCircles.transition()
        .attr("cx", function(d) { return xScale(d.date); })
        .attr("cy", function(d) { return d.y; });

    // Remove old event circles
    eventCircles.exit().remove();


    
    // Data-join (enter, update, exit) for the event text
    var eventText = svg.selectAll(".event-text")
        .data(events, function(d) { return d.date; }); // Use date as key for binding data

    // Enter new event text
    eventText.enter()
        .append("text")
        .attr("class", "event-text")
        .attr("x", function(d) { return xScale(d.date) + 10; }) // Offset text to the right of the dot
        .attr("y", function(d) { return d.y + 4; }) // Offset text to align it with the dot center
        .text(function(d) { return d.event; })
        .call(wrapText, width - 10 - 10); // Subtract the padding and a little extra for margin; 

    // Update existing event text
    svg.selectAll(".event-text")
        ...
        .text(function(d) { return d.event; })
        .call(wrapText, width - margin.right - 10); // Ensure the wrapText function takes margins into account


    // Remove old event text
...
function parseFirstYear(yearRange) {
    var match = yearRange.match(/(\d{4})/); // Matches the first sequence of 4 digits
    return match ? parseYear(match[0]) : null;
}       

        // Initialize timeline with the first country in the list
        updateTimeline(data[0].country);